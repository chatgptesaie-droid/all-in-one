      ...
      await fetchValidationChunk(
        ...
        (event) => {
          if (event.type === "init") {
            ...
            if (finished) {
              statusMessage = `Terminé - ${event.valid} valides, ${event.invalid} invalides`;
              running = false;
              notify({ type: "done", valid: event.valid, invalid: event.invalid });
              try {
                const valid = getValidResults();
                window.localStorage.setItem("crunchyroll-validator-valid-results", JSON.stringify(valid));
              } catch {}
            }
...
export function stopValidation() {
  ...
  controller = null;
  running = false;
  statusMessage = "Validation arrêtée";
  notify({ type: "stop" });
}
