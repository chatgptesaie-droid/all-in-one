...
export function subscribe(subscriber: (event: Event) => void) {
...
export async function startValidation(cookieText: string) {
  ...
  try {
    ...
    while (!finished && !controller.signal.aborted) {
      await fetchValidationChunk("/api/lovable", cookieText, start, limit, controller.signal, (event) => {
        if (event.type === "init") {
          ...
          if (finished) {
            state.statusMessage = `Terminé - ${event.valid} valides, ${event.invalid} invalides`;
            notify(event);
          }
        }
      });
...
export function stopValidation() {
  ...
  if (state.running) {
    state.running = false;
    state.statusMessage = "Validation arrêtée";
    notify({ type: "stop" });
  }
}
