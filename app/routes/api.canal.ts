import { parseCanalCookiesFromText, validateCanalBatch } from "~/lib/canal.server";
...
export async function action({ request }: { request: Request }) {
  ...
  const batches = parseCanalCookiesFromText(text);
  if (!batches.length) return Response.json({ type: "error", message: "Aucun cookie Canal+ valide trouvé" }, { status: 400 });
