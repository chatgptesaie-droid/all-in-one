...
import * as validator from "../lib/crunchyroll.validator";
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="text-xl font-semibold tracking-tight" style={{ color: "var(--text)" }}>
              Crunchyroll Cookie Checker
            </h1>
            <p className="text-sm mt-0.5" style={{ color: "var(--text-muted)" }}>
              Validation des cookies Crunchyroll
            </p>
          </div>
          <div className="flex items-center gap-2">
            {isValidating && <span className="w-2 h-2 rounded-full bg-orange-500 animate-pulse" />}
        ...
        <div className="flex-1 flex flex-col overflow-hidden min-w-0">
          ...
          {results.length === 0 && !isValidating && (
            <div className="shrink-0 border-b px-4 py-4 sm:px-6" style={{ background: "var(--bg-surface-alt)", borderColor: "var(--border)" }}>
              ...
              <textarea
                ...
                placeholder={`.crunchyroll.com\tTRUE\t/\tTRUE\t0\tetp_rt\tvaleur...`}
                      ...
                      {results.map((result, idx) => (
                        ...
                        >
                          ...
                          <td className="px-3 py-2.5 text-xs" style={{ color: "var(--text)" }}>
                            {(result.accountInfo?.display_name as string) || (result.accountInfo?.email as string) || "-"}
                          </td>
          ...
          {selectedResult && (
            <div className="border-t lg:hidden overflow-y-auto max-h-[55vh]" style={{ background: "var(--bg-surface)", borderColor: "var(--border)" }}>
              ...
              <CrunchyrollDetails result={selectedResult} />
            </div>
          )}
...
function CrunchyrollDetails({ result }: { result: ValidationResult }) {
  ...
  return (
    <div className="p-4 space-y-4 sm:p-5 sm:space-y-5">
      ...
      {/* Debug info — visible si invalide */}
      {!result.isValid && result.accountInfo && Object.keys(result.accountInfo).length > 0 && (
        <section>
          ...
          <div className="rounded-lg border divide-y" style={{ background: "var(--bg)", borderColor: "var(--border)" }}>
            {result.cookiesData.map((cookie, idx) => (
              <div key={idx} className="px-3 py-2 flex items-center gap-3" style={{ borderColor: "var(--border-subtle)" }}>
                <span className="text-[11px] text-orange-500/80 font-medium shrink-0 w-[130px] truncate">{cookie.name}</span>
