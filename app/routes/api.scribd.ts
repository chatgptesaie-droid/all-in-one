import { parseScribdCookiesFromText, validateScribdBatch } from "~/lib/scribd.server";
...
export async function action({ request }: { request: Request }) {
  ...
  const batches = parseScribdCookiesFromText(cookieText);
  if (!batches.length) return Response.json({ type: "error", message: "Aucun cookie Scribd valide trouvé" }, { status: 400 });
