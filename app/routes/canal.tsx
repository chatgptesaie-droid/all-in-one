...
import * as validator from "../lib/canal.validator";
import type { CanalResult } from "../lib/canal.validator";

export function meta() {
  return [
    { title: "Canal+ Cookies Validator" },
    { name: "description", content: "Validateur de cookies Canal+" },
  ];
}
...
export default function CanalPage() {
  ...
  const [results, setResults] = useState<CanalResult[]>([]);
  const [selected, setSelected] = useState<CanalResult | null>(null);
    ...
    try {
      if (isFolder) {
        ...
        const contents = await Promise.all(files.map(async (file) => {
          const response = await fetch("/api/storage/content", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ bucket: STORAGE_BUCKET, path: file.path }) });
        ...
        const response = await fetch("/api/storage/content", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ bucket: STORAGE_BUCKET, path }) });
    ...
    () =>
      validator.subscribe((event) => {
        ...
        if (event.type === "done") {
          setRunning(false);
          setStatus(
            `Terminé - ${event.valid} valides, ${event.invalid} invalides`,
          );
        }
        if (event.type === "error") setStatus(`Erreur: ${event.message}`);
        if (event.type === "stop" || event.type === "stopped")
          setRunning(false);
      }),
    [],
  );
    ...
    (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      void Promise.all(files.map((file) => file.text())).then((contents) => {
        setCookieText(
          contents
            .map(
              (content, index) => `# --- ${files[index].name} ---\n${content}`,
            )
            .join("\n\n"),
        );
        setFileLoaded(
  ...
  const isExpired = (result: CanalResult) =>
    result.accountInfo?.expiry_status === "expired";
  const expired = results.filter(isExpired);
  const exportResults = (json: boolean) => {
    if (!valid.length) return;
    download(
      json
        ? JSON.stringify(
            { exportTime: new Date().toISOString(), results: valid },
            null,
            2,
          )
        : `# Netscape HTTP Cookie File\n\n${valid.map((result) => result.netscapeFormat).join("\n\n")}`,
      json ? "application/json" : "text/plain",
      `canal_valid_cookies.${json ? "json" : "txt"}`,
    );
  };
    ...
    >
      ...
      >
        <div className="flex items-center justify-between gap-3">
          <div>
            <h1 className="text-xl font-semibold">Canal+ Cookies Validator</h1>
            <p className="mt-1 text-sm">
              Validation du compte, de l'offre et des profils
            </p>
          </div>
          <span className="text-xs">
            {running ? "Validation en cours..." : status}
          </span>
        </div>
      </header>
      <div className="flex flex-1 overflow-hidden min-h-0">
        <main className="flex-1 flex flex-col overflow-hidden min-w-0">
          ...
          >
            <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center">
              ...
              <button
                ...
                onClick={start}
                disabled={running || !cookieText.trim()}
              >
              ...
              {running && (
                <button
                  ...
                  onClick={() => validator.stopValidation()}
                >
                  Arrêter
                </button>
              )}
          ...
          {results.length === 0 && !running && (
            ...
            >
              <label className="block text-xs font-semibold mb-2 uppercase tracking-wide">
                Cookies Canal+ au format Netscape
              </label>
              <textarea
                value={cookieText}
                onChange={(event) => setCookieText(event.target.value)}
                placeholder=".canalplus.com\tTRUE\t/\tTRUE\t0\tpassId\tvaleur..."
                ...
                <table className="min-w-[850px] w-full text-sm">
                  <thead>
                    ...
                    >
                      ...
                      <th className="px-3 py-2.5">Profils</th>
                    ...
                    {results.map((result) => (
                      ...
                      >
                        ...
                        <td className="px-3 py-2.5 text-xs">
                          {result.accountInfo?.["Nom"] || "-"}
                        </td>
                        <td className="px-3 py-2.5 text-xs">
                          {result.accountInfo?.["Offre"] || "-"}
                        </td>
                        <td className="px-3 py-2.5 text-xs">
                          {result.accountInfo?.["Date d'échéance"] || "-"}
                        </td>
                        <td className="px-3 py-2.5 text-xs">
                          {result.accountInfo?.profile_count ?? "-"}
                        </td>
                        <td
          ...
          {results.length > 0 && (
            ...
            >
              Total: <b>{results.length}</b> · Valides:{" "}
              <b className="text-emerald-500">{valid.length}</b> · Invalides:{" "}
              <b className="text-red-400">
                {results.length - valid.length - expired.length}
              </b> · Expirés:{" "}
              <b className="text-orange-400">{expired.length}</b>
            </footer>
          )}
        </main>
        <aside
          ...
          {selected ? (
            <>
              ...
              <div className="flex items-center justify-between mt-4 mb-2">
                ...
                <button
                  className="text-xs"
                  onClick={() => copyCookie(selected.netscapeFormat)}
                >
                  Copier
                </button>
              </div>
