...
/**
 * Prétraite le texte de cookies Crunchyroll :
 * - Convertit "#HttpOnly_.domain" en ".domain" pour que le parser Netscape standard les accepte
 * - Si le texte ne contient pas déjà des marqueurs "# ---", enveloppe tout en un seul batch
 */
function preprocessCrunchyrollCookies(raw: string): string {
  const processed = raw
    .split("\n")
    .map((line) => {
      // "#HttpOnly_.crunchyroll.com\t..." → ".crunchyroll.com\t..."
      if (line.startsWith("#HttpOnly_")) {
        return line.replace(/^#HttpOnly_/, "");
      }
      return line;
    })
    .join("\n");

  // Si pas déjà groupé avec des marqueurs de fichier, on enveloppe en un seul batch
...
export async function action({ request }: { request: Request }) {
  ...
  const batches = parseCookiesFromText(preprocessCrunchyrollCookies(cookieText));

  if (batches.length === 0) {
    return new Response(
      JSON.stringify({ type: "error", message: "Aucun cookie valide trouvé dans le texte" }),
      { status: 400, headers: { "Content-Type": "application/json" } }
    );
  }
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      for (let i = startIndex; i < endIndex; i++) {
        ...
        try {
          const result = await validateCrunchyrollBatch(batches[i]);
