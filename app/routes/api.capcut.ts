import { parseCapcutCookiesFromText, validateCapcutBatch } from "~/lib/capcut.server";
...
export async function action({ request }: { request: Request }) {
  ...
  const batches = parseCapcutCookiesFromText(cookieText);
  ...
  const limitValue = typeof body.limit === "number" && Number.isFinite(body.limit) ? body.limit : 20;
